...
ipcRenderer.on('db-path', (event, data) => {
    ...
    exports.getVentas = (dateStart, dateEnd) => {
        ...
        let stringQuery = "SELECT v.ID,v.Date, v.Quantity, v.Price, p.Price_Sell,v.FK_Product idProducto, p.Price Precio_Produccion, p.Name,p.Available,v.ID_Factura  FROM VENTAS v INNER JOIN Productos p ON v.FK_Product = p.ID WHERE Date >= '" + dateStart + "' and Date <= '" + dateEnd + "'"
        let query = db.prepare(stringQuery);
        const row = query.all();
        return row
    }

    exports.getVentasByReceipt = (factura) => {
        let stringQuery = "SELECT v.ID,v.Date, v.Quantity, v.Price, p.Price_Sell, v.FK_Product idProducto, p.Price Precio_Produccion, p.Name,p.Available,v.ID_Factura FROM VENTAS v INNER JOIN Productos p ON v.FK_Product = p.ID WHERE v.ID_Factura = " + factura
        let query = db.prepare(stringQuery);
        const row = query.all();
        return row
    }

    exports.getLastVentas = () => {
        let stringQuery = "SELECT * FROM Ventas ORDER BY id DESC LIMIT 1;"
        let query = db.prepare(stringQuery);
        const row = query.all();
        return row
    }
    ...
    exports.createVentas = (producto) => {
        ...
        const result = query.run(producto.ID, date.toString(), producto.Cantidad, producto.Total, producto.ID_Factura)
